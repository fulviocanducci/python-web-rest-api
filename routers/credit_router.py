...
def get_credit_by_id(id: int, db: Session = Depends(get_db)):
    repo = CreditRepository(db)
...
def post_credit(model: CreditCreate, db: Session = Depends(get_db)):
    repo = CreditRepository(db)
...
def put_credit(model: CreditUpdate, id: int, db: Session = Depends(get_db)):
    repo = CreditRepository(db)
...
def delete_credit(id: int, db: Session = Depends(get_db)):
    repo = CreditRepository(db)
    credit: Credit = repo.get_by_id(id)
    if not credit:
        raise HTTPException(status_code=404, detail="Not found")
